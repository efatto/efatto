...
class AccountAssetAsset(orm.Model):
    ...
    def _get_depreciation_stop_date(self, cr, uid, asset,
        ...
        if dl_ids:
            for dl in dl_ids:
                #TODO calcola il valore che dovrebbe essere stato ammortizzato con la percentuale standard
                #TODO trova la differenza e dividila per la % standard = anni in più o in meno (se negativa)
                if dl.type == 'depreciate':
                    depreciated_amount += dl.amount
                    depreciated_year += 1
            if depreciated_year > 0:
                standard_depreciation_amount = asset.asset_value * asset.method_percent / 100.0
                diff = standard_depreciation_amount * depreciated_year - depreciated_amount
                if diff != 0:
                    lost_years = int(Decimal(str(diff / standard_depreciation_amount)).quantize(Decimal('0')))
        ...
        if asset.method_time == 'year':
            depreciation_stop_date = depreciation_start_date + \
                relativedelta(years=(int(asset.method_number) + lost_years), days=-1)
        return depreciation_stop_date

    def compute_depreciation_board(self, cr, uid, ids, context=None):
        super(AccountAssetAsset, self).compute_depreciation_board(cr, uid, ids, context)
        ...
        for asset in self.browse(cr, uid, ids, context):
            depreciation_start_line_id = adl_obj.search(cr, uid, [('asset_id', '=', asset.id), ('type', '=', 'depreciate'),], order='line_date asc', limit=1)
            if depreciation_start_line_id:
                depreciation_start_date = adl_obj.browse(cr, uid, depreciation_start_line_id, context)[0].line_date[:4] + '-01-01'
            last_depreciation_id = adl_obj.search(cr, uid, [('asset_id', '=', asset.id), ('type', '=', 'depreciate'),], order='line_date desc', limit=1)
            if last_depreciation_id:
                adl = adl_obj.browse(cr, uid, last_depreciation_id, context)[0]
                last_depreciation_date = datetime.strptime((adl.line_date), '%Y-%m-%d')
            depreciation_stop_date = self._get_depreciation_stop_date(cr, uid, asset, datetime.strptime(depreciation_start_date, '%Y-%m-%d'), context)
            if depreciation_stop_date > last_depreciation_date:
                vals = {
                    ...
                    'asset_id': asset.id,
                    'name': adl.name,
                    'line_date': adl.line_date, #.strftime('%Y-%m-%d'), # mettere l'anno successivo
                }
        ...
        if asset.method_time == 'year':
            ...
            if asset.first_year_half:
                if residual_amount == amount_to_depr:
                    year_amount_linear = amount_to_depr * asset.method_percent / 100.0 / 2.0

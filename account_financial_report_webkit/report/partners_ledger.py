...
class PartnersLedgerWebkit(report_sxw.rml_parse,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if self.pool['ir.module.module'].search(
            self.cr, self.uid, [
                ('name', '=', 'partner_subaccount'),
                ('state', '=', 'installed')]):
        ...
        if partner_ids:
            ...
            # SC: filter only on accounts of all partner, to exclude account
            # of type receivable or payable not linked to partners
            partner_ids = self.pool['res.partner'].search(
                self.cr, self.uid, [('parent_id', '=', False)], {})
            partners = self.pool['res.partner'].browse(
                self.cr, self.uid, partner_ids, {})
            if 'receivable' in filter_type:
                accounts = [x.property_account_receivable.id for x in partners]
            ...
            for res_line in res_lines:
                ...
                if inv and res_line.get('id', False) in [
                    ...
                    if res_line['rec_id'] == line['rec_id']:
                        ...
                        a += 1
                        res_lines.remove(res_line)
                elif line['lname'] == res_line['lname'] \
                        or res_line['lname'] == line['lname'] or \
                        line.get('invoice_number', False) and \
                        line['invoice_number'] == res_line['lname']:
                    inv_lines.insert(i+a+1, res_line)
                    a += 1
                    res_lines.remove(res_line)
        if len(res_lines) > 0:

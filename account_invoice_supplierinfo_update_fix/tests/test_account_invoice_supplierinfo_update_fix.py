...
class AccountInvoiceSupplierinfoUpdateFix(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
            ...
            [
                {
                    ...
                    "date": "2025-06-23",
                    "invoice_date": cls.used_date + timedelta(days=-5),
        ...
        cls.invoice._sync_dynamic_lines(
            container={"records": cls.invoice, "self": cls.invoice}
        )
    ...
    def test_00_supplierinfo_update(self):
        ...
        supplierinfo = self.product.seller_ids.filtered(
            lambda x: x.partner_id == self.vendor
        )
    ...
    def test_01_supplierinfo_create(self):
        ...
        supplierinfo.date_end = self.used_date + timedelta(days=-10)
        self.invoice.button_cancel()
        self.invoice.button_draft()
        invoice_form = Form(self.invoice)
        invoice_form.supplierinfo_ok = False
        invoice_form.invoice_date = self.used_date + timedelta(days=-5)
    ...
    def test_02_supplierinfo_update_past(self):
        ...
        supplierinfo.date_start = self.used_date + timedelta(days=-100)
        supplierinfo.date_end = self.used_date + timedelta(days=-10)
        self.invoice.button_cancel()
        self.invoice.button_draft()
        invoice_form = Form(self.invoice)
        invoice_form.supplierinfo_ok = False
        invoice_form.invoice_date = self.used_date + timedelta(days=-20)
        with invoice_form.invoice_line_ids.edit(0) as invoice_line:
        ...
        # changed from v. 12.0: check there is only supplierinfo, as the invoice is
        # the same, with a changed price
        self.assertEqual(len(supplierinfos.ids), 1)

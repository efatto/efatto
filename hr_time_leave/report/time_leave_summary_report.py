...
def emp_create_xml(self, cr, uid, dept, leave_type, row_id, empid, name, som, eom):
    display={}
    if dept==0:
        count=0
        registry = openerp.registry(cr.dbname)
        holidays_ids = registry['hr.time_leave'].search(cr, uid, [('employee_id','in',[empid,False]), ('type', '=', 'remove')])
        ids_date = registry['hr.time_leave'].read(cr, uid, holidays_ids, ['time_from','time_to','time_leave_status_id','state'])
        ...
        for index in range(1,61):
            ...
            for item in ids_date:
                if current >= strToDate(item['time_from']) and current <= strToDate(item['date_to']):
                    if item['state'] in leave_type:
                        display[index]=item['time_leave_status_id'][0]
                        count=count +1
                    else:
                        display[index]=' '
                    break
...
class report_custom(report_rml):
    def create_xml(self, cr, uid, ids, data, context):
        ...
        depts=[]
        emp_id={}
        rpt_obj = registry['hr.time_leaves']
        ...
        cr.execute("SELECT id, name, color_name FROM hr_time_leaves_status ORDER BY id")
        legend=cr.fetchall()
        today=datetime.datetime.today()

        first_date=data['form']['time_from']
        ...
        if data['form']['leave_type']!='both':
            type=data['form']['leave_type']
            if data['form']['leave_type']=='Confirmed':
...
report_custom('report.time_leave.summary', 'hr.time_leave', '', 'addons/hr_time_leave/report/time_leave_summary.xsl')

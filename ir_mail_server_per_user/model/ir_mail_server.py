...
class ir_mail_server(osv.Model):
    ...
    _columns = {
        'user_id_ept': fields.many2one(
            'res.users', 'User',
            help='When user is assigned then at the time of email sending '
                 'from Odoo if no outgoing server found then '
        ...
        if not mail_server and user_id:
            ...
            if email :
                mail_server_ids = self.search(
                    cr,SUPERUSER_ID,[('user_id_ept','=',user_id)],
                    order='sequence', limit=1)
                if mail_server_ids :
                    mail_server = mail_server_ids[0]
        return super(ir_mail_server,self).send_email(
            cr,uid,message,mail_server_id=mail_server)

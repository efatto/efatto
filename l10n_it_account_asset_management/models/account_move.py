...
class account_move(orm.Model):
    ...
    #verify # REMOVED from 6.1 because block "post" and "cancel" action, not "unlink" action
    def write(self, cr, uid, ids, vals, context=None):
        super(account_move, self).write(cr, uid, ids, vals, context=context)
...
class account_move_line(orm.Model):
    ...
    _columns = {
        # verify if needed 'subsequent_asset': fields.boolean('Subsequent Purchase of Asset'),
    }
        ...
        if len(tax_list) == 2:
            for tax in tax_list:
                if tax.get('balance', False):
                    ...
                    ind_amount = float(
                        Decimal(str(ind_tax['amount'])).quantize(
                            Decimal('1.00'), rounding=ROUND_HALF_UP))
        asset_value = vals['debit'] + ind_amount or \
            - vals['credit'] - ind_amount
        return asset_value

    def create(self, cr, uid, vals, context=None, check=True):
        res = super(account_move_line, self).create(
            cr, uid, vals, context, check)
        asset_obj = self.pool.get('account.asset.asset')
        move = self.pool.get('account.move').browse(cr, uid, vals['move_id'])
        journal_obj = self.pool['account.journal']
        if vals.get('asset_category_id') and vals.get('asset_id') and not context.get('create_move_from_button'):
            if journal_obj.browse(cr, uid, [move.journal_id.id], context=context)[0].type in (
                    'sale', 'purchase_refund', 'sale_refund', 'purchase'):
                ...
                move_line_ids = self.search(cr, uid, [('asset_id', '=', vals['asset_id'])])
                ...
                if len(move_line_ids) > 1:
                    ...
                    #  update value of newly created asset
                    asset_obj.write(cr, uid, [vals['asset_id']], {'purchase_value': asset_value}, context=context)
#TODO: UPDATE VALUE FROM MOVE
#             else:
#                 if asset_value > 0.0:
...
#                     continue
# 
#                 if journal_obj.browse(
#                        cr, uid, [aml.move_id.journal_id.id], context=context)[
#                        0].type in ('sale', 'sale_refund', 'purchase', 'purchase_refund'):

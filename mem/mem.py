...
class mem_vehicle_cost(osv.Model):
    _name = 'mem.vehicle.cost'
    ...
    def _set_odometer(self, cr, uid, id, name, value, args=None, context=None):
        ...
        odometer_id = self.pool.get('mem.vehicle.odometer').create(cr, uid, data, context=context)
    ...
    _columns = {
        ...
        'vehicle_id': fields.many2one('mem.vehicle', 'Vehicle', required=True, help='Vehicle concerned by this log'),
        'cost_subtype_id': fields.many2one('mem.service.type', 'Type', help='Cost type purchased with this cost'),
        ...
        'parent_id': fields.many2one('mem.vehicle.cost', 'Parent', help='Parent cost to this current cost'),
        'cost_ids': fields.one2many('mem.vehicle.cost', 'parent_id', 'Included Services'),
        'odometer_id': fields.many2one('mem.vehicle.odometer', 'Odometer', help='Odometer measure of the vehicle at the moment of this log'),
        ...
        'contract_id': fields.many2one('mem.vehicle.log.contract', 'Contract', help='Contract attached to this cost'),
    ...
    def create(self, cr, uid, data, context=None):
        ...
        if 'contract_id' in data and data['contract_id']:
            contract = self.pool.get('mem.vehicle.log.contract').browse(cr, uid, data['contract_id'], context=context)
        ...
        if 'odometer' in data and not data['odometer']:
            ...
            del(data['odometer'])
        return super(mem_vehicle_cost, self).create(cr, uid, data, context=context)


class mem_vehicle_tag(osv.Model):
    _name = 'mem.vehicle.tag'
    _columns = {
        'name': fields.char('Name', required=True, translate=True),
    }

class mem_vehicle_state(osv.Model):
    _name = 'mem.vehicle.state'
    _order = 'sequence asc'
    _columns = {
        'name': fields.char('Name', required=True),
        'sequence': fields.integer('Sequence', help="Used to order the note stages")
    }
    _sql_constraints = [('mem_state_name_unique','unique(name)', 'State name already exists')]


class mem_vehicle_model(osv.Model):
    ...
    def on_change_brand(self, cr, uid, ids, model_id, context=None):
        ...
        brand = self.pool.get('mem.vehicle.model.brand').browse(cr, uid, model_id, context=context)
        return {
            'value': {
    ...
    _columns = {
        ...
        'brand_id': fields.many2one('mem.vehicle.model.brand', 'Model Brand', required=True, help='Brand of the vehicle'),
        'vendors': fields.many2many('res.partner', 'mem_vehicle_model_vendors', 'model_id', 'partner_id', string='Vendors'),
...
class mem_contract_state_vehicle_model_brand(osv.Model):
    _name = 'mem.vehicle.model.brand'
    ...
    _columns = {
        ...
        'image_medium': fields.function(_get_image, fnct_inv=_set_image,
            ...
            store = {
                'mem.vehicle.model.brand': (lambda self, cr, uid, ids, c={}: ids, ['image'], 10),
            },
...
class mem_vehicle(osv.Model):
    ...
    def return_action_to_open(self, cr, uid, ids, context=None):
        ...
        if context.get('xml_id'):
            res = self.pool.get('ir.actions.act_window').for_xml_id(cr, uid ,'mem', context['xml_id'], context=context)
            res['context'] = context
    ...
    def act_show_log_cost(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        res = self.pool.get('ir.actions.act_window').for_xml_id(cr, uid ,'mem','mem_vehicle_costs_act', context=context)
        res['context'] = context
        res['context'].update({
    ...
    def _get_odometer(self, cr, uid, ids, odometer_id, arg, context):
        ...
        for record in self.browse(cr,uid,ids,context=context):
            ids = self.pool.get('mem.vehicle.odometer').search(cr, uid, [('vehicle_id', '=', record.id)], limit=1, order='value desc')
            if len(ids) > 0:
                res[record.id] = self.pool.get('mem.vehicle.odometer').browse(cr, uid, ids[0], context=context).value
        return res
    ...
    def _set_odometer(self, cr, uid, id, name, value, args=None, context=None):
        if value:
            ...
            return self.pool.get('mem.vehicle.odometer').create(cr, uid, data, context=context)
    ...
    def _search_get_overdue_contract_reminder(self, cr, uid, obj, name, args, context):
        ...
        for field, operator, value in args:
            ...
            cr.execute('select cost.vehicle_id, count(contract.id) as contract_number FROM mem_vehicle_cost cost left join mem_vehicle_log_contract contract on contract.cost_id = cost.id WHERE contract.expiration_date is not null AND contract.expiration_date < %s AND contract.state IN (\'open\', \'toclose\') GROUP BY cost.vehicle_id', (today,))
    ...
    def _search_contract_renewal_due_soon(self, cr, uid, obj, name, args, context):
        ...
        for field, operator, value in args:
            ...
            cr.execute('select cost.vehicle_id, count(contract.id) as contract_number FROM mem_vehicle_cost cost left join mem_vehicle_log_contract contract on contract.cost_id = cost.id WHERE contract.expiration_date is not null AND contract.expiration_date > %s AND contract.expiration_date < %s AND contract.state IN (\'open\', \'toclose\') GROUP BY cost.vehicle_id', (today, limit_date))
            res_ids = [x[0] for x in cr.fetchall()]
            res.append(('id', search_operator, res_ids))
        return res
            ...
            for element in record.log_contracts:
                if element.state in ('open', 'toclose') and element.expiration_date:
                    ...
                    if overdue or due_soon:
                        ids = self.pool.get('mem.vehicle.log.contract').search(cr,uid,[('vehicle_id', '=', record.id), ('state', 'in', ('open', 'toclose'))], limit=1, order='expiration_date asc')
                        if len(ids) > 0:
                            #we display only the name of the oldest overdue/due soon contract
                            name=(self.pool.get('mem.vehicle.log.contract').browse(cr, uid, ids[0], context=context).cost_subtype_id.name)
    ...
    def _get_default_state(self, cr, uid, context):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mem', 'vehicle_state_active')
        except ValueError:
            model_id = False
        return model_id
    
    def _count_all(self, cr, uid, ids, field_name, arg, context=None):
        Odometer = self.pool['mem.vehicle.odometer']
        LogFuel = self.pool['mem.vehicle.log.fuel']
        LogService = self.pool['mem.vehicle.log.services']
        LogContract = self.pool['mem.vehicle.log.contract']
        Cost = self.pool['mem.vehicle.cost']
        return {
            vehicle_id: {
    ...
    _columns = {
        ...
        'model_id': fields.many2one('mem.vehicle.model', 'Model', required=True, help='Model of the vehicle'),
        'log_fuel': fields.one2many('mem.vehicle.log.fuel', 'vehicle_id', 'Fuel Logs'),
        'log_services': fields.one2many('mem.vehicle.log.services', 'vehicle_id', 'Services Logs'),
        'log_contracts': fields.one2many('mem.vehicle.log.contract', 'vehicle_id', 'Contracts'),
        ...
        'state_id': fields.many2one('mem.vehicle.state', 'State', help='Current state of the vehicle', ondelete="set null"),
        ...
        'tag_ids' :fields.many2many('mem.vehicle.tag', 'mem_vehicle_vehicle_tag_rel', 'vehicle_tag_id','tag_id', 'Tags', copy=False),
    ...
    def on_change_model(self, cr, uid, ids, model_id, context=None):
        if not model_id:
            return {}
        model = self.pool.get('mem.vehicle.model').browse(cr, uid, model_id, context=context)
        return {
            'value': {
    ...
    def create(self, cr, uid, data, context=None):
        ...
        vehicle_id = super(mem_vehicle, self).create(cr, uid, data, context=context)
        vehicle = self.browse(cr, uid, vehicle_id, context=context)
        self.message_post(cr, uid, [vehicle_id], body=_('%s %s has been added to the mem!') % (vehicle.model_id.name,vehicle.license_plate), context=context)
        return vehicle_id
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for vehicle in self.browse(cr, uid, ids, context):
            ...
            if 'model_id' in vals and vehicle.model_id.id != vals['model_id']:
                value = self.pool.get('mem.vehicle.model').browse(cr,uid,vals['model_id'],context=context).name
            ...
            if 'state_id' in vals and vehicle.state_id.id != vals['state_id']:
                value = self.pool.get('mem.vehicle.state').browse(cr,uid,vals['state_id'],context=context).name
        ...
        vehicle_id = super(mem_vehicle,self).write(cr, uid, ids, vals, context)
        return True


class mem_vehicle_odometer(osv.Model):
    _name='mem.vehicle.odometer'
    ...
    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context=None):
        if not vehicle_id:
            return {}
        odometer_unit = self.pool.get('mem.vehicle').browse(cr, uid, vehicle_id, context=context).odometer_unit
        return {
            'value': {
                'unit': odometer_unit,
            }
    ...
    _columns = {
        ...
        'vehicle_id': fields.many2one('mem.vehicle', 'Vehicle', required=True),
...
class mem_vehicle_log_fuel(osv.Model):

    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context=None):
        if not vehicle_id:
            return {}
        vehicle = self.pool.get('mem.vehicle').browse(cr, uid, vehicle_id, context=context)
    ...
    def _get_default_service_type(self, cr, uid, context):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mem', 'type_service_refueling')
        except ValueError:
            model_id = False
        return model_id

    _name = 'mem.vehicle.log.fuel'
    _description = 'Fuel log for vehicles'
    _inherits = {'mem.vehicle.cost': 'cost_id'}
    ...
    _columns = {
        ...
        'cost_id': fields.many2one('mem.vehicle.cost', 'Cost', required=True, ondelete='cascade'),
...
class mem_vehicle_log_services(osv.Model):
    ...
    def _get_default_service_type(self, cr, uid, context):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mem', 'type_service_service_8')
        except ValueError:
            model_id = False
        return model_id

    _inherits = {'mem.vehicle.cost': 'cost_id'}
    _name = 'mem.vehicle.log.services'
...
class mem_service_type(osv.Model):
    _name = 'mem.service.type'
...
class mem_vehicle_log_contract(osv.Model):
    ...
    def scheduler_manage_auto_costs(self, cr, uid, context=None):
        ...
        vehicle_cost_obj = self.pool.get('mem.vehicle.cost')
        d = datetime.datetime.strptime(fields.date.context_today(self, cr, uid, context=context), tools.DEFAULT_SERVER_DATE_FORMAT).date()
        contract_ids = self.pool.get('mem.vehicle.log.contract').search(cr, uid, [('state','!=','closed')], offset=0, limit=None, order=None,context=None, count=False)
        ...
        for contract in self.pool.get('mem.vehicle.log.contract').browse(cr, uid, contract_ids, context=context):
            ...
            while (startdate <= d) & (startdate <= datetime.datetime.strptime(contract.expiration_date, tools.DEFAULT_SERVER_DATE_FORMAT).date()):
                ...
                cost_id = self.pool.get('mem.vehicle.cost').create(cr, uid, data, context=context)
                startdate += deltas.get(contract.cost_frequency)
        return True
    ...
    def scheduler_manage_contract_expiration(self, cr, uid, context=None):
        ...
        for vehicle, value in res.items():
            self.pool.get('mem.vehicle').message_post(cr, uid, vehicle, body=_('%s contract(s) need(s) to be renewed and/or closed!') % (str(value)), context=context)
    ...
    def act_renew_contract(self, cr, uid, ids, context=None):
        ...
        for element in self.browse(cr, uid, ids, context=context):
            ...
            newid = super(mem_vehicle_log_contract, self).copy(cr, uid, element.id, default, context=context)
        mod, modid = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mem', 'mem_vehicle_log_contract_form')
        return {
            'name':_("Renew Contract"),
            'view_mode': 'form',
            'view_id': modid,
            'view_type': 'tree,form',
            'res_model': 'mem.vehicle.log.contract',
    ...
    def _get_default_contract_type(self, cr, uid, context=None):
        try:
            model, model_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mem', 'type_contract_leasing')
        except ValueError:
            model_id = False
        return model_id
    ...
    _inherits = {'mem.vehicle.cost': 'cost_id'}
    _name = 'mem.vehicle.log.contract'
    ...
    _columns = {
        ...
        'generated_cost_ids': fields.one2many('mem.vehicle.cost', 'contract_id', 'Generated Costs'),
...
class mem_contract_state(osv.Model):
    _name = 'mem.contract.state'

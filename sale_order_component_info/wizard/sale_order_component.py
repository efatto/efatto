...
class SaleComponent(models.TransientModel):
    ...
    def _get_components(self):
        ...
        product = sale_order_line.product_id.id \
            if sale_order_line.product_id.type != 'service' else False
        if product:
            ...
            for child_product in child_dict:
                ...
                parent_id = [
                    y['parent_id'] for y in child_products
                    if y['product_id'] == child_product
                    and y['parent_id'] != product
                ]
                if parent_id:
                    product_dict.update({
...
class SaleComponentLine(models.TransientModel):
    ...
    def _get_bookmarked_components(self):
        ...
        for product in self.env['product.product'].browse(sol_res.keys()):
            ...
            for x in child_products:
                if x['product_id'] not in child_dict:
                    child_dict.update({x['product_id']: x['qty'] * float_round(
    ...
    def _compute_qty_to_deliver(self):
        for component in self:
            ...
            component.display_qty_widget = (component.state == 'draft'
            ...
            for line in lines:
                ...
                if virtual_available_at_date < line.qty_to_deliver:
                    if virtual_available_at_date >= 0.0:
                        missing_qty = \
                            line.qty_to_deliver - virtual_available_at_date
                    else:
                        missing_qty = line.qty_to_deliver
                    missing_component_qty = bookmarked_component_dict.get(
                        product.id, 0.0) + missing_qty
    ...
    def _compute_is_mto(self):
        ...
        for line in self:
            ...
            # Check MTO
            mto_route = line.sale_order_line_id.order_id.warehouse_id.mto_pull_id.\
                route_id
            if not mto_route:
                try:

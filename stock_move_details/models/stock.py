    ...
    def _set_sign_product_qty(self):
        for move in self:
            move.qty_signed = move.product_uom_qty * (
                -1
                if move.location_dest_id.usage
                ...
                if move.location_dest_id.usage == move.location_id.usage
                else 1
            )

    qty_signed = fields.Float(
        compute=_set_sign_product_qty, store=True, group_operator="sum"
    )
    ...
    def _set_sign_product_qty(self):
        for line in self:
            line.qty_signed = line.qty_done * (
                -1
                if line.location_dest_id.usage
                in ["customer", "inventory", "production", "supplier"]
                ...
                if line.location_dest_id.usage == line.location_id.usage
                else 1
            )
